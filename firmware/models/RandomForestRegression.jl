...
function RandomForestRegression(k, X_train, y_train, X_test, y_test, wholedata)
    ...
    for i in 1:k_fold
        ...
        rfr = machine(RandomForestRegressor(), temp_X_train, vec(Matrix(temp_y_train)))
        MLJ.fit!(rfr, verbosity = 0)

        temp_predict_y_train = MLJ.predict(rfr, temp_X_train)
        temp_predict_y_test = MLJ.predict(rfr, temp_X_test)
