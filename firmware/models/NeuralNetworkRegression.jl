...
function NeuralNetworkRegression(k, X_train, y_train, X_test, y_test, wholedata)
    ...
    # Training model
    model = machine(NeuralNetworkRegressor(builder=MLJFlux.MLP(hidden=(128,64), σ = Flux.σ), optimiser=Flux.ADAM(0.001), loss=Flux.mse, epochs=16, 
    batch_size=6, rng=StableRNG(42)), X_train, vec(Matrix(y_train)))
