...
function SVRRegression(k, X_train, y_train, X_test, y_test, wholedata)
    ...
    for i in 1:k_fold
        ...
        svr = machine(SVMRegressor(), temp_X_train, vec(Matrix(temp_y_train)))
        MLJ.fit!(svr, verbosity = 0)

        temp_predict_y_train = MLJ.predict(svr, temp_X_train)
        temp_predict_y_test = MLJ.predict(svr, temp_X_test)
