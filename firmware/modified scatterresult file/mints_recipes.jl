...
@recipe function f(inputs::ScatterResult;)
    ...
    scalefactor = maxval/20
    @series begin
        seriestype := :density
        subplot := 1
        xlims := (minval, maxval)
        ylims := (-0.1 / scalefactor, 1.1*maximum(maximum.([ky.density, kytest.density])))
        legend := false
        xlabel := ""
        ylabel := ""
        color --> mints_palette[1]
    ...
    @series begin
        ...
        subplot := 3
        orientation := :h
        xlims := (-0.1 / scalefactor, 1.1*maximum(maximum.([kŷ.density, kŷtest.density])))
        ylims := (minval, maxval)
        legend := false
        xlabel := ""
        ylabel := ""
